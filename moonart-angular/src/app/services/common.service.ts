...
export class CommonService {
    ...
    constructor(
        ...
        this._router.events
            ...
            .subscribe((events: RoutesRecognized[]) => {
                ...
                if (
                    events[0].urlAfterRedirects !== events[1].urlAfterRedirects
                ) {
                    // We make sure to not add to the history the same webpage if it's reloaded
    ...
    getUserConfig() {
        // Gets the getConfig returned object (AJAX request from user.service), and stores it
        const token = this._userService.getToken();
        let config = this._sharedService.defaultConfig;

        if (token) {
            this._userService.getConfig().subscribe(
                (response: { status: string; config: Config }) => {
    ...
    setUserConfig(data) {
        ...
        this._userService.setConfig(data).subscribe(
            (response) => {
                if (!response.status || response.status != "error") {
                    console.log("setUserConfig", response);
                    const config = {
    ...
    formatText(str) {
        ...
        while (search != -1) {
            ...
            if (foundAt === 999) foundAt = str.length;
            ...
            if (found) {
                // Won't let me inject without Sanitizer :(
    ...
    redirectToProfile(event) {
        ...
        if (link) this._router.navigateByUrl("/profile/" + link);
    }
            ...
            if (type === "timestamp") {
                ...
                if (lang === 1)
                    str =
                        ...
                        mydate.getFullYear() +
                        " at " +
                        ("0" + mydate.getHours()).substr(-2) +
                        ":" +
                        ("0" + mydate.getMinutes()).substr(-2);
                else if (lang === 2)
                    str =
                        mydate.getDate() +
                        " de " +
                ...
                if (delta < 60) return secondsStr;
                else if (minutes < 60) return minutesStr;
                else if (hours < 24) return hoursStr;
                else if (days < 7) return daysStr;
                else if (weeks < 54) return weeksStr;
                else return yearsStr;
            }
        }
        return str;
    }
    ...
    displayNotification(message, permanent, arr, ref = null, resolve = null) {
        ...
        setTimeout(
            function () {
                alert = <HTMLElement>(
                    document.querySelectorAll(`${alertType}`)[0]
                );
                ...
                function () {
                    try {
                        ...
                        setTimeout(
                            ...
                            500,
                            alert
                        ); // Once the opacity animation has ended, we clear its value.
                    } catch (err) {}
                },
                3000,
                alert
