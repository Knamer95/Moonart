...
export class UserEditComponent implements OnInit {
    ...
    onSubmit() {
            ...
            for (let i = 0; i < this.user.description.length; i++) {
                if (this.user.description.charCodeAt(i) == 10) {
                    this.user.description =
                        this.user.description.substr(0, i) +
                        "\\n" +
                        this.user.description.substr(i + 1);
                }
            }
            this.user.description = this._commonService.noscript(
                this.user.description
            );
        }
        ...
        if (
            this.imageSize == true &&
            (this.pwdChange == false || this.newPwd == true)
        ) {
            this.changeUserSettings(this);
        }
    }
    ...
    changeUserSettings(e) {
        this._userService.update(this.token, this.user).subscribe(
            (response) => {
                ...
                let message =
                    response.status === "success"
                        ? this.currentLang.attributes.profileUpdateSuccess
                        : this.currentLang.attributes.profileUpdateError;
                this.emitter.emit({
                    type: emitterTypes.ALERT,
                    status: response.status,
                    notificationType: response.status,
    ...
    nameGen() {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            randomName.push(String.fromCharCode(letter));
        }
        randomName.push("-");
        for (let i = 0; i < 6; i++) {
            let number = Math.random() * 9 + 48;
            randomName.push(String.fromCharCode(number));
        }
